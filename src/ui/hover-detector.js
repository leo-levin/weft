...
class HoverDetector {
    ...
    lines.forEach((line, lineIndex) => {
      parameterNames.forEach(paramName => {
        ...
        while ((match = directRegex.exec(line)) !== null) {
          ...
          console.log(`🔍 Checking '${paramName}' at column ${match.index} in line: '${line}'`);
          console.log(`🔍 Before char: '${beforeChar}', Inside brackets: ${insideBrackets}, Is param def: ${isParameterDefinition}`);
          
          // Skip if this is part of @paramName pattern, but allow parameter definitions
          if (beforeChar !== '@' && (!insideBrackets || isParameterDefinition)) {
            occurrences.push({
              paramName,
              line: lineIndex,
              column: match.index,
              ...
              text: match[0],
              type: isParameterDefinition ? 'parameter_definition' : 'direct'
            });
            console.log(`✅ Found ${isParameterDefinition ? 'parameter definition' : 'direct parameter reference'}: '${paramName}' at line ${lineIndex + 1}`);
          } else {
            if (beforeChar === '@') {
              console.log(`⚪ Skipping parameter '${paramName}' at column ${match.index} - part of strand access (@${paramName})`);
            } else if (insideBrackets && !isParameterDefinition) {
              console.log(`⚪ Skipping parameter '${paramName}' at column ${match.index} - inside non-parameter brackets`);
            }
          }
        }
        ...
        while ((match = strandRegex.exec(line)) !== null) {
          ...
          if (matchesParameterPragma) {
            ...
            console.log(`✅ Found parameter strand access: '${fullMatch}' (instance: ${instanceName}, strand: ${paramName}) at line ${lineIndex + 1}`);
          } else {
            console.log(`⚪ Skipping non-parameter strand access: '${fullMatch}' (instance: ${instanceName}, strand: ${paramName})`);
          }
        }
      });
    });
    
    console.log('📋 All parameter occurrences found:', occurrences);
    return occurrences;
  }
  ...
  createHoverZones(text, occurrences) {
    console.log(`🎨 Creating ${occurrences.length} hover zones`);
    ...
    console.log(`📐 Editor measurements:`, { fontSize, lineHeight, paddingLeft, paddingTop });
    ...
    occurrences.forEach((occ, index) => {
      ...
      console.log(`✅ Created hover zone ${index + 1}: '${occ.text}' at line ${occ.line + 1}`);
    });
  }
  ...
  createPositionedHoverZone(occurrence, measurements) {
    ...
    console.log(`📍 Positioning '${occurrence.text}': left=${left}px, top=${top}px, width=${width}px`);
    ...
    hoverZone.addEventListener('mouseenter', (e) => {
      console.log(`🎯 Hover zone mouseenter for '${occurrence.paramName}'`);
    ...
    hoverZone.addEventListener('mouseleave', (e) => {
      console.log(`👋 Hover zone mouseleave for '${occurrence.paramName}'`);
  ...
  handleParameterHover(event, paramName) {
    console.log(`🎯 Parameter hover detected for '${paramName}'`, event.target);
    
    clearTimeout(this.hoverTimeout);
    
    this.hoverTimeout = setTimeout(() => {
      console.log(`⏰ Hover timeout triggered for '${paramName}'`);
      const paramStrand = this.env.getParameterStrand(paramName);
      console.log(`🔍 Parameter strand lookup result for '${paramName}':`, paramStrand);
      
      if (paramStrand) {
        // Calculate position for popover
        const rect = event.target.getBoundingClientRect();
        console.log(`📐 Popover target position:`, rect);
        
        const popoverTarget = {
          getBoundingClientRect: () => rect
        };
        
        console.log(`🚀 Showing popover for '${paramName}'`);
        this.widgetManager.showPopover(paramName, popoverTarget);
      } else {
        console.log(`❌ No parameter strand found for '${paramName}'`);
      }
    }, 150); // Short delay to prevent accidental triggers
  }
  
  handleParameterLeave(event) {
    console.log(`👋 Parameter leave detected`);
    clearTimeout(this.hoverTimeout);
    
    // Hide popover after a longer delay to allow moving to popover
    this.hideTimeout = setTimeout(() => {
      console.log(`⏰ Hide timeout triggered - checking if should hide popover`);
